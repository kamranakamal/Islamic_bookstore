...
export function BookCard({ book }: BookCardProps) {
  return (
    <article className="flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition hover:shadow">
      <Link href={`/books/${book.slug}`} className="relative h-56 w-full">
        <Image
          src={book.coverUrl}
          alt={book.title}
          fill
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div>
          <Link href={`/books/${book.slug}`} className="text-lg font-semibold text-gray-900">
            {book.title}
          </Link>
        ...
        <div className="mt-auto flex items-center justify-between text-sm text-gray-700">
          <span className="font-semibold text-primary">{book.priceFormatted}</span>
