...
export function AdminLoginForm({ redirectTo }: AdminLoginFormProps) {
  ...
  const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (event) => {
    ...
    try {
      ...
      const typedProfile = profile as Pick<ProfileRow, "role"> | null;

      if (profileError || !typedProfile || typedProfile.role !== "admin") {
        await supabase.auth.signOut();
        setState("error");
        setError({ message: "Your account does not have admin access." });
        return;
      }
      ...
      if (data.session) {
        ...
        // Give server time to persist session cookie before navigating
